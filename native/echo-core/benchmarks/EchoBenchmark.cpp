...
Options parseOptions(const int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--cycles" && index + 1 < argc)
            ...
            std::cout << "Usage: mechana_echo_benchmark [--cycles N] [--seconds N] [--sample-rate HZ]\n";
            std::exit(0);
        } else
...
Measurement measure(mechana::echo::EchoEngine& engine, const std::span<const float> source,
                    const std::size_t blocks, const mechana::echo::Parameters& parameters) {
    engine.reset();
...
int main(const int argc, char** argv) {
    try {
        ...
        std::cout << std::fixed << std::setprecision(3)
                  ...
                  << "effect-name=Modeled_Echo effect-id=modeled-echo model=Analog_Memory architecture="
        ...
        (void) measure(engine, source, std::min<std::size_t>(blocks, 32), parameters);

        std::vector<double> loads;
        for (std::size_t cycle = 1; cycle <= options.cycles; ++cycle) {
            const auto measurement = measure(engine, source, blocks, parameters);
