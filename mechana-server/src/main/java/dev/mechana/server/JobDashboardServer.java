...
public final class JobDashboardServer implements AutoCloseable {
	...
	private static final String HTML = """
			...
			<section><div class="bar"><div class="fill" id="overallBar"></div></div><p id="summary"></p><div id="jobDetails"></div><p><button id="pauseJobButton" hidden>Pause job</button> <button id="resumeJobButton" hidden>Resume job</button> <button id="resumeAsNewButton" hidden>Resume as new job</button> <button id="abortJobButton" hidden>Abort job</button></p><p class="FAILED" id="error"></p></section>
			...
			<section id="artifactSection" hidden><h2>Artifacts</h2><div class="muted">These files remain available after a server restart.</div><ul id="artifacts"></ul><button id="showArtifacts" hidden>Show in Finder</button> <button id="purge" hidden>Purge job and artifacts</button></section>
			<section><h2>Recent events</h2><ul id="events"></ul></section><script>
			const esc=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
			const fields=o=>Object.entries(o||{}).map(([k,v])=>`<span><b>${esc(k)}:</b> ${k==='resumedFromJobId'?`<a href="/dashboard/jobs/${encodeURIComponent(v)}"><code>${esc(v)}</code></a>`:esc(v)}</span>`).join('<br>');
			const size=n=>n<1024?n+' B':n<1048576?(n/1024).toFixed(1)+' KB':(n/1048576).toFixed(1)+' MB';
			async function refresh(){try{const s=await fetch('__STATUS_URL__',{cache:'no-store'}).then(r=>r.json());stage.textContent=s.stage;overall.textContent=s.progress+'%';workers.textContent=s.activeWorkers+' / '+s.configuredWorkers;elapsed.textContent=s.elapsed;overallBar.style.width=s.progress+'%';summary.textContent=`${s.completedWorkUnits} of ${s.totalWorkUnits} work units complete`;error.textContent=s.error||'';identity.innerHTML=`Job: <code>${esc(s.jobId)}</code> · Plugin: <code>${esc(s.plugin)}</code>`;jobDetails.innerHTML=fields(s.details);workUnits.innerHTML=s.workUnits.map(x=>`<tr><td><b>${esc(x.label)}</b><br><code>${esc(x.id)}</code></td><td>${fields(x.details)}</td><td><code>${esc(x.workerAddress)}</code></td><td class="${x.state}">${x.state}</td><td><div class="bar"><div class="fill" style="width:${x.progress}%"></div></div> ${x.progress}%</td><td>${x.elapsed}</td></tr>`).join('');events.innerHTML=s.events.map(e=>`<li>${esc(e)}</li>`).join('');pauseJobButton.hidden=!s.pausable;pauseJobButton.onclick=async()=>{const r=await fetch(`/api/jobs/${encodeURIComponent(s.jobId)}/pause`,{method:'POST'});if(!r.ok)throw new Error(await r.text());await refresh()};resumeJobButton.hidden=!s.resumable;resumeJobButton.onclick=async()=>{const r=await fetch(`/api/jobs/${encodeURIComponent(s.jobId)}/resume`,{method:'POST'});if(!r.ok)throw new Error(await r.text());await refresh()};resumeAsNewButton.hidden=!s.resumableAsNew;resumeAsNewButton.onclick=async()=>{const r=await fetch(`/api/jobs/${encodeURIComponent(s.jobId)}/resume-as-new`,{method:'POST'});if(!r.ok)throw new Error(await r.text());const j=await r.json();location.href=`/dashboard/jobs/${encodeURIComponent(j.jobId)}`};abortJobButton.hidden=!s.abortable;abortJobButton.onclick=async()=>{if(!confirm(`Abort job ${s.jobId}? Running workers may take a moment to stop.`))return;const r=await fetch(`/api/jobs/${encodeURIComponent(s.jobId)}/abort`,{method:'POST'});if(!r.ok)throw new Error(await r.text());await refresh()};artifactSection.hidden=!s.completed;artifacts.innerHTML=(s.artifacts||[]).map(a=>`<li><a href="${esc(a.url)}" download>${esc(a.name)}</a> <span class="muted">(${size(a.size)})</span></li>`).join('')||'<li class="muted">No downloadable artifacts.</li>';showArtifacts.hidden=!s.completed;showArtifacts.onclick=async()=>{const r=await fetch(`/api/jobs/${encodeURIComponent(s.jobId)}/reveal-artifacts`,{method:'POST'});if(!r.ok)throw new Error(await r.text())};purge.hidden=!s.completed;purge.onclick=async()=>{if(!confirm(`Permanently delete job ${s.jobId} and all of its local artifacts?`))return;const r=await fetch(`/api/jobs/${encodeURIComponent(s.jobId)}`,{method:'DELETE'});if(!r.ok)throw new Error(await r.text());location.href='/dashboard'};}catch(e){error.textContent=e;}setTimeout(refresh,1000)}refresh();
			</script></body></html>
			""";
}
