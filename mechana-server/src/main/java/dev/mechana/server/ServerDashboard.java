...
final class ServerDashboard {
	...
	private static final String HTML = """
			...
			const capabilitySummary=items=>{const plugins=items.filter(item=>!item.startsWith('sandbox.'));const backend=items.find(item=>item.startsWith('sandbox.backend.'));const sandbox=backend?.includes('linux-bubblewrap')?'Linux sandbox':backend?.includes('macos-')?'macOS sandbox':backend?.includes('windows-appcontainer')?'Windows sandbox':backend?'Sandboxed':'';return [`${plugins.length} plugin${plugins.length===1?'':'s'}`,sandbox].filter(Boolean).join(' · ')};
			const capabilityDetails=items=>{const names={'sleep':'Sleep','video-ffmpeg':'FFmpeg video','fractal-render':'Fractal renderer','ocr-tesseract':'Tesseract OCR','blender-render':'Blender renderer'};const plugins=items.filter(item=>!item.startsWith('sandbox.')).map(item=>names[item]||item).sort();return plugins.length?`Plugins: ${plugins.join(', ')}`:'No computation plugins'};
