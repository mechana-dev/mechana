...
final class ServerDashboard {
	...
	private static final String HTML = """
			...
			const capabilitySummary=items=>{const plugins=items.filter(item=>!item.startsWith('sandbox.'));const backend=items.find(item=>item.startsWith('sandbox.backend.'));const sandbox=backend?.includes('linux-bubblewrap')?'Linux sandbox':backend?.includes('macos-')?'macOS sandbox':backend?'Sandboxed':'';return [`${plugins.length} plugin${plugins.length===1?'':'s'}`,sandbox].filter(Boolean).join(' · ')};
