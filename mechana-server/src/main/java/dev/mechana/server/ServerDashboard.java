...
final class ServerDashboard {
	...
	private static final String HTML = """
			...
			</style></head><body><h1>Mechana Server</h1><div class="muted">Live workers and job history</div><p><button id="restartServerButton">Restart server</button> <button id="stopServerButton">Stop server</button></p>
			...
			const pluginCapabilities=items=>items.filter(item=>!item.startsWith('sandbox.')&&!item.startsWith('storage.'));
			const capabilitySummary=items=>{const plugins=pluginCapabilities(items);const direct=items.includes('storage.client-direct-artifacts.v1')?'Direct transfer':'';const backend=items.find(item=>item.startsWith('sandbox.backend.'));const sandbox=backend?.includes('linux-bubblewrap')?'Linux sandbox':backend?.includes('macos-')?'macOS sandbox':backend?.includes('windows-appcontainer')?'Windows sandbox':backend?'Sandboxed':'';return [`${plugins.length} plugin${plugins.length===1?'':'s'}`,direct,sandbox].filter(Boolean).join(' · ')};
			const capabilityDetails=items=>{const names={'sleep':'Sleep','video-ffmpeg':'FFmpeg video','fractal-render':'Fractal renderer','ocr-tesseract':'Tesseract OCR','blender-render':'Blender renderer','audio-convolution-reverb':'Convolution reverb','audio-ir-deconvolution':'Create impulse response'};const plugins=pluginCapabilities(items).map(item=>names[item]||item).sort();const transport=items.includes('storage.client-direct-artifacts.v1')?' · Transport: Direct client artifacts':'';return (plugins.length?`Plugins: ${plugins.join(', ')}`:'No computation plugins')+transport};
			...
			async function purgeAllJobs(){if(!confirm('Permanently delete all completed jobs and all of their local artifacts?'))return;const r=await fetch('/api/jobs/completed',{method:'DELETE'});if(!r.ok)throw new Error(await r.text());await refresh(false)}
			restartServerButton.onclick=async()=>{if(!confirm('Restart the Mechana server? Active in-memory jobs will be lost; workers will reconnect automatically.'))return;const previousPid=serverPid.textContent;restartServerButton.disabled=true;restartServerButton.textContent='Restarting…';const r=await fetch('/api/server/restart',{method:'POST'});if(!r.ok){restartServerButton.disabled=false;restartServerButton.textContent='Restart server';throw new Error(await r.text())}for(let attempt=0;attempt<60;attempt++){await new Promise(resolve=>setTimeout(resolve,500));try{const status=await fetch('__STATUS_URL__',{cache:'no-store'}).then(response=>response.ok?response.json():Promise.reject());if(String(status.serverPid)!==previousPid){location.reload();return}}catch(ignored){}}restartServerButton.disabled=false;restartServerButton.textContent='Restart server';error.textContent='Server did not become ready within 30 seconds.'};
			stopServerButton.onclick=async()=>{if(!confirm('Stop the Mechana server? Workers will disconnect until the Server app is launched again.'))return;stopServerButton.disabled=true;stopServerButton.textContent='Stopping…';const r=await fetch('/api/server/stop',{method:'POST'});if(!r.ok){stopServerButton.disabled=false;stopServerButton.textContent='Stop server';throw new Error(await r.text())}location.href='mechana://server-stopped'};
			purgeAllButton.onclick=purgeAllJobs;
			async function refresh(schedule=true){try{const s=await fetch('__STATUS_URL__',{cache:'no-store'}).then(r=>r.json());serverPid.textContent=s.serverPid;serverDate.textContent=s.serverDate;serverTime.textContent=s.serverTime;serverUptime.textContent=s.serverUptime;connected.textContent=s.connectedWorkers;registered.textContent=s.registeredWorkers;active.textContent=s.activeJobs;completed.textContent=s.completedJobs;purgeAllButton.hidden=s.completedJobs===0;noWorkers.hidden=s.workers.length>0;noActiveJobs.hidden=s.activeJobItems.length>0;noCompletedJobs.hidden=s.completedJobItems.length>0;workers.innerHTML=s.workers.map(w=>`<tr><td><code>${esc(w.id)}</code></td><td><code>${esc(w.address)}</code></td><td class="${esc(w.state)}">${esc(w.state)}</td><td>${esc(w.activity)}</td><td>${w.jobId?w.progress+'%':'—'}</td><td>${w.jobId?`<a href="/dashboard/jobs/${encodeURIComponent(w.jobId)}"><code>${esc(w.jobId)}</code></a>`:'—'}</td><td title="${esc(capabilityDetails(w.capabilities))}">${esc(capabilitySummary(w.capabilities))}</td><td>${esc(w.lastSeen)}</td></tr>`).join('');activeJobRows.innerHTML=s.activeJobItems.map(j=>row(j)+`<td>${j.stage==='PAUSED'?`<button onclick="resumeJob('${esc(j.jobId)}')">Resume</button>`:`<button onclick="pauseJob('${esc(j.jobId)}')">Pause</button>`} <button onclick="abortJob('${esc(j.jobId)}')">Abort</button></td></tr>`).join('');completedJobRows.innerHTML=s.completedJobItems.map(j=>row(j)+`<td>${esc(terminalTime(j))}</td><td>${j.stage==='CANCELLED'||j.stage==='FAILED'?`<button onclick="resumeAsNew('${esc(j.jobId)}')">Resume as new</button> `:''}<button onclick="purgeJob('${esc(j.jobId)}')">Purge</button></td></tr>`).join('');error.textContent='';}catch(e){error.textContent=e;}if(schedule)setTimeout(refresh,1000)}refresh();
			</script></body></html>
			""";
}
